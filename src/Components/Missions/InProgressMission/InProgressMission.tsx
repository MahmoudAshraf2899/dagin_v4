...
export const InProgressMission = () => {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    API.get(
      `dashboard/missions?status=in_progress&sort_by=id&sort_order=DESC&page=${pageNumber}&limit=${pageSize}`
    ).then((res) => {
      if (res) {
        if (res.status === 403) {
  ...
  const hanldeChangePage = (targetPN: number) => {
    setIsLoading(true);
    API.get(
      `dashboard/missions?status=in_progress&sort_by=id&sort_order=DESC&page=${targetPN}&limit=${pageSize}`
    ).then((res) => {
      if (res) {
        if (res.status === 403) {
          toast.error(" عفوا انت ليس لديك صلاحية الوصول لهذه الصفحة ");
          ...
          <div className="grid grid-cols-2 mr-4 ml-4 mt-4 mission-content">
            ...
            <div className="col-start-2 flex justify-end">
              <div className="flex gap-4 mission-status">
                <div className="inprogress">تحت التنفيذ</div>
